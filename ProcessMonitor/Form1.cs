    ...
    public partial class Form1 : Form
    {
        ...
        private void GetAllProcess()
        {
            ...
            foreach (Process p in myProcess)
            {
                ...
                row.Cells[2].Value = String.Format("{0:###,##0.00}MB", p.WorkingSet64 / 1024.0f / 1024.0f);//为进程分配的物理内存量
                //有些进程无法获取启动时间和文件名信息，所以要用try/catch
                try
                {
                    row.Cells[3].Value = String.Format("{0}", p.StartTime);//启动时间
                    row.Cells[4].Value = p.MainModule.FileName;//文件信息
                }
                catch
                {
